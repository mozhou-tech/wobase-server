...
- **Next.js 14+ (App Router)**：全栈 React 框架，API Routes + Server Actions
...
> **Edge Functions 替代方案**：使用 Next.js API Routes (`app/api/*`) 或 Server Actions 处理支付回调、账务结算、异步任务。如需边缘部署，使用 Vercel Edge Runtime。
...
3. 初始化应在 `join_app` API Route 中统一完成，避免客户端直接操作
...
// 2. 小程序登录（需配合后端）
// 小程序调用 wx.login 获取 code，传给 API Route
const { data: session } = await fetch('/api/auth/wechat-miniprogram', {
  method: 'POST',
  body: JSON.stringify({ code: wxLoginCode, app_id: 'your_app_uuid' })
}).then(r => r.json());
```

**`wechat_miniprogram_login` API Route**：
...
如需同步到应用层 `user_profiles`，在 `join_app` API Route 中读取 `auth.users.user_metadata` 写入。
...
1. 在 **服务端或 API Route** 签发/刷新会话时，将当前环境写入 **`auth.users` 的 `app_metadata`（或等价不可由终端用户随意篡改的声明）**，例如 `app_metadata.env = 'prod'`。
...
**推荐方案**：通过 API Route `join_app(app_id, env)` 在事务内统一初始化：
...
> **SECURITY DEFINER 与 EXECUTE**：`SECURITY DEFINER` 函数以定义者权限执行，**禁止**默认对 `PUBLIC`/`authenticated` 随意开放 `EXECUTE`。推荐：`REVOKE ALL ON FUNCTION deduct_balance(...) FROM PUBLIC;`，仅 `GRANT EXECUTE` 给 **`service_role`**（由 API Route / Server Action 调用），或由仅具高权限角色的后台用户执行。若业务上必须由终端用户直连调用，则函数内须强制 `auth.uid()` 与账户 `user_id` 一致（如上），并仍需最小化授予对象。
>
...
- **占席总数不超过 `seat_limit`**：须在 **`SECURITY DEFINER` 函数或 API Route** 内 `SELECT ... FOR UPDATE` 锁定 `team_vip_subscriptions` 行后，统计 `status = 'active'` 条数再允许插入 / 激活；禁止仅靠客户端校验。
...
#### 服务端职责（API Routes / Server Actions）

- 开通 / 续费 / 解约团队订阅（支付回调幂等、`biz_no`）
- **分配席位 / 撤销席位**（强制席位上限与订阅窗口校验）
- 订阅到期批量撤销占席并刷新 `app_users`
- **档位合成**：与个人 `paid` / `user_vips` 叠加时执行下文「`paid` 与 `team` 并存」规则；所有写入 `app_users` 的路径收口到少量 API Routes / Server Actions / DB 函数，避免分叉逻辑
...
2. **定时补偿修正（第二道防线）**
   使用 **pg_cron**（Supabase 支持）或 **API Route + Cron Trigger**（如 Vercel Cron），定期扫描过期记录并修正：
...
可对 **`roles.code = 'super_admin'`** 等保留短路分支（仍需写入审计）；若团队倾向集中管控，也可让**全部管理写路径**仅通过 **Server Action + service_role**，服务端校验运营 JWT 与权限表后再写入，数据库侧对 `authenticated` **不开放**管理员类策略——二者择一或组合，须在评审中固化。
...
- **避免敏感字段泄露**：Realtime 推送会携带整行数据。以下表**强烈建议禁用 Realtime**或建立字段级过滤机制：
  ...
  > 若业务确实需要 Realtime 推送敏感表的变更，应通过 Server Action 作为中转，由服务端过滤敏感字段后再推送至客户端。
...
### API Route / Server Action 接口清单

| API Route / Server Action | 方法 | 功能 | 调用方 | 幂等键 |
|---|---|---|---|---|
...
- **支付回调（API Routes）**：必须实现幂等校验（`biz_no` 去重）和请求签名验证，防止重放攻击。
- **登录/注册接口**：利用 Supabase Auth 内置的 rate limiting（如邮件验证码频次限制）。
- **自定义 API（API Routes）**：对敏感操作（如高频查询、批量导出）增加 IP / 用户级 rate limiting，可使用 Vercel Edge Config 或 Redis 计数器。
...
- 应通过环境变量注入 API Routes / 服务端，不在代码仓库中硬编码。
