...
5. **`auth.users` 删除的级联风险**
   当前大量表对 `auth.users.id` 使用 `ON DELETE CASCADE`：
   - `user_profiles` → `auth.users`
   - `app_users` → `auth.users`
   - `wallet_accounts` → `auth.users`
   - `user_vips` → `auth.users`

   **风险**：在 Supabase Auth 中删除一个用户，会级联删除该用户在**所有应用**的全部数据。这在多应用平台中是危险操作。

   **建议**：
   - 核心表改为 `ON DELETE RESTRICT` 或 `ON DELETE SET NULL`
   - 用户注销通过 Edge Function 流程化处理：
     1. 标记 `app_user_status = 'deleted'`（各应用独立软删除）
     2. 7 天后由后台任务清理敏感数据
     3. 最后才删除 `auth.users`
